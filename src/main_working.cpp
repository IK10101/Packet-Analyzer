...
int main(int argc, char* argv[]) {
    ...
    for (int i = 3; i < argc; i++) {
        ...
        if (arg == "--block-ip" && i + 1 < argc) {
            ...
            matcher.addPattern(argv[++i]);
            std::cout << "[Rules] Added pattern: " << argv[i] << "\n";
        }
    }
    ...
    while (reader.readNextPacket(raw)) {
        ...
        // Pattern matching on HTTP payload
        if (matcher.hasPatterns() && parsed.has_tcp && parsed.dest_port == 80) {
            size_t payload_offset = 14;
            uint8_t ip_ihl = raw.data[14] & 0x0F;
            payload_offset += ip_ihl * 4;
            if (payload_offset + 12 < raw.data.size()) {
                ...
                if (payload_offset < raw.data.size()) {
                    size_t payload_len = raw.data.size() - payload_offset;
                    auto result = matcher.scan(raw.data.data() + payload_offset, payload_len);
                    if (result.matched) {
                        for (const auto& p : result.matched_patterns) {
                            if (std::find(flow.matched_patterns.begin(),
                                          flow.matched_patterns.end(), p)
                                == flow.matched_patterns.end()) {
                                flow.matched_patterns.push_back(p);
                            }
                        }
                        std::cout << "[PATTERN MATCH] " << parsed.src_ip
                                  << " -> " << parsed.dest_ip
                                  << " matched: ";
                        for (const auto& p : result.matched_patterns)
                            std::cout << "\"" << p << "\" ";
    ...
    // Build JSON data
    std::vector<JsonReporter::FlowInfo> flow_list;
    for (const auto& [tuple, flow] : flows) {
        JsonReporter::FlowInfo fi;
        fi.src_ip = std::to_string((flow.tuple.src_ip) & 0xFF) + "." + std::to_string((flow.tuple.src_ip >> 8) & 0xFF) + "." + std::to_string((flow.tuple.src_ip >> 16) & 0xFF) + "." + std::to_string((flow.tuple.src_ip >> 24) & 0xFF);
        fi.dst_ip = std::to_string((flow.tuple.dst_ip) & 0xFF) + "." + std::to_string((flow.tuple.dst_ip >> 8) & 0xFF) + "." + std::to_string((flow.tuple.dst_ip >> 16) & 0xFF) + "." + std::to_string((flow.tuple.dst_ip >> 24) & 0xFF);
        fi.src_port = flow.tuple.src_port;
        fi.dst_port = flow.tuple.dst_port;
        fi.app = appTypeToString(flow.app_type);
        fi.sni = flow.sni;
        fi.packets = flow.packets;
        fi.blocked = flow.blocked;
        fi.matched_patterns = flow.matched_patterns;
        flow_list.push_back(fi);
    }
    std::vector<std::pair<std::string,uint64_t>> app_stats_named;
    for (const auto& [app, count] : sorted_apps) {
        app_stats_named.push_back({appTypeToString(app), count});
    }
    JsonReporter::generate(output_file + ".report.json", total_packets, forwarded, dropped, app_stats_named, flow_list);
